...
if [ -n "$3" ]; then
  if [ -f "$3" ]; then
    ...
    echo "Wrong path to wikidata subclass list! See '--help' for usage information!"
    exit 1
  fi
fi
...
if [ -n "$4" ]; then
  if [ -f "$4" ]; then
    ...
    echo "Wrong path to wikidata transitive closure! See '--help' for usage information!"
    exit 1
  fi
fi
...
END
  # copy part
  if [ "$(echo -n $val | wc -m)" -eq "1" ]; then
    rsync "$prefix""0""$val" "$host:/tmp/$USER/types/$1/"
  else
    rsync "$prefix$val" "$host:/tmp/$USER/types/$1/"
  fi
  # increase part value
  val="$(echo "$val + 1" | bc)"
done
...
for host in $(cat $host_list); do
  rsync "$host:/tmp/$USER/types/$1/tcs/*" "/tmp/$USER/types/result/$1/"
done
